...
def replace_space(lines: List, sub: str, max_vallen: int):
...
class SimCollector(BaseCollector):
    ...
    @classmethod
    def arrange_p(cls, dict_p, symbols: Symbols):
        p_lines = list()
        max_vallen = 0
        for i, (name, value) in enumerate(dict_p.items()):
            symbols.set_index(name, i)
            # '//' is a comment format in Rust lang.
            p_lines.append(f"{value},***space***// p[{i}] {name}\n")
            vallen = len(str(value))
            max_vallen = vallen if max_vallen < vallen else max_vallen

        p_lines = replace_space(p_lines, ",", max_vallen)
        return "".join(p_lines)
